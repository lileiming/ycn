        ...
        for Y in range(self.cardTypeNum):
            if (self.Card != []):
                ...
                for X in range(self.total):
                    if (self.data.iloc[12,X+2] == -1):
                        self.data = self.data.sort_values(by=str(self.colName[X+2]), ascending=False)
                        #print(self.colName[X + 2])
                        nowCard = self.data.iloc[Y, 1]
                        if nowCard in self.Card:
                            nowIndex = self.Card.index(nowCard)
                            #print(nowIndex)
                            self.data.iloc[self.cardTypeNum, X+2] = self.data.iloc[Y, 1]
