...
class Windows_NODE(YokoCustomlibrary.FILE_NODE):
    ...
    def open_combox(self, *args):
        ...
        self.var_col = list(pd.ExcelFile(var_file_path).parse(var_sheet_name,header=self.var_header_T).keys())
    ...
    def open_combox2(self,*args):
        ...
        self.var_col2 = list(pd.ExcelFile(var_file).parse(var_sheet,header=self.var_header_R).keys())
