...
def func_getHtmlList(url):
    try:
        headers = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) '
                        'Chrome/79.0.3845.74 Safari/537.36  Edg/89.0.774.63'}
...
def func_download_wallpaper(filepath,var_pic):
    file_name = var_pic.split('OHR.')[-1]  # 获取图片文件名
...
if __name__ == "__main__":
    try:
        var_page_num = int(input("请输入要下载的页数（1页12张）："))
    except ValueError :
        var_page_num = 1
    for var_num in range(var_page_num):
        ...
        for var_pic_addr in list_pic_result:
            ...
            var_pic_new_addr = f'https://cn.bing.com/th?id=OHR.{var_pic_name}'
            func_download_wallpaper(filepath = var_subdir, var_pic = var_pic_new_addr)
